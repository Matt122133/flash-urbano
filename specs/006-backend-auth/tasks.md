...
- [x] T025 [US1] Verificar a mano con el servicio apagado: cargar calle y esquina, ver punto y precio, sin errores en consola (SC-001, quickstart paso 1) **Verificado el 2026-08-11 con el servicio realmente apagado** —comprobado desde la sesión: `curl` a `/salud` sin respuesta— y el precio se calculó igual. Es la promesa de FR-001 medida en las condiciones que la hacen valer, no por analogía. La navegación muestra "Ingresar" en vez del nombre, y **es correcto**: sin el servicio el sitio no puede resolver la credencial, y prefiere no mostrar identidad antes que mostrar una equivocada.
...
- [x] T055 [US3] Implementar `POST /auth/codigo` y `POST /auth/codigo/verificar` en `backend/internal/auth/handlers.go` y cablearlos en `backend/cmd/api/main.go`. En éxito: marca usado, crea el usuario si no existe, emite la misma sesión larga que Google **Hecho**, en `handlers_codigo.go` aparte de `handlers.go`: el camino por mail necesita tres piezas —códigos, límites y enviador— que el de Google no usa, y juntarlos le haría cargar dependencias ajenas. `POST /auth/codigo` responde **204 sin cuerpo pase lo que pase** desde la validación de forma hacia abajo. En éxito, `verificar` marca usado, crea el usuario **después** de validar (al revés, cualquiera fabricaría filas con direcciones ajenas) y emite **la misma sesión larga** que Google. **Sin precargar nombre** (FR-021a). Cableado en `main.go`, y de paso **la purga de códigos vencidos quedó enganchada al janitor** — sin eso la tabla crecería para siempre y es un fallo que no avisa.
- [x] T056 [US3] Probar en `backend/internal/auth/handlers_test.go` que `POST /auth/codigo` **responde exactamente lo mismo** exista o no el usuario, y también cuando el límite está excedido (FR-014), y que hacia afuera un código incorrecto y uno vencido son indistinguibles **Hecho: ocho pruebas contra Postgres real, cero skips.** Que `POST /auth/codigo` responde idéntico exista o no el usuario **y con el límite excedido** —con el control positivo de que en el primer caso sí se mandaron los dos mails, sin el cual un handler que no hiciera nada pasaría perfecto—, que un código incorrecto y uno vencido son indistinguibles hacia afuera **y distinguibles en el rastro** (FR-022d), que una dirección que nunca pidió código tampoco se delata, que una dirección con forma inválida sí da 400 sin mandar mail, y que **un fallo del proveedor no cambia la respuesta** — el precio declarado de FR-014. Más `TestElCaminoCompletoEntrega`, control positivo de todo el archivo: se pide, llega el código al doble, se verifica y sale sesión con usuario nuevo y sin nombre. **Verificado por mutación**: haciendo que el límite conteste 429 en vez de 204, la prueba se pone roja mostrando las dos respuestas lado a lado.
- [x] T057 [P] [US3] Implementar los dos pasos del camino por mail en `web/components/sesion/ingreso-por-codigo.tsx`: pedir el código y escribirlo, con reintento y un mensaje útil cuando el mail no llega, sin confirmar si la dirección está registrada **Hecho.** Dos pasos, y **nunca dice si la dirección está registrada** (FR-014): el servicio responde 204 pase lo que pase y la pantalla se comporta igual — siempre pasa al paso del código. El precio, asumido en el copy: cuando el mail **no llega**, el sitio no puede explicar por qué, así que el segundo paso lleva ayuda concreta —puede tardar un minuto, mirá en spam, revisá la dirección— en vez de un "revisá tu correo" que deja a la persona sin nada que hacer. **Se nombra spam explícitamente** porque hoy, con el dominio recién estrenado, es donde el código termina en algunos proveedores. El reenvío tiene una **cuenta regresiva de 30 s**, y no es cosmética: el límite del servicio son 3 cada 15 minutos y **no se puede ver desde afuera** —responde igual—, así que sin frenarlo acá la persona gasta sus tres intentos tocando el botón mientras el mail viaja. Un error incorrecto, vencido o agotado dan **el mismo mensaje**, igual que el servicio.
- [x] T058 [US3] Sumar el camino por mail a `web/app/ingresar/page.tsx`, y el alta de nombre y teléfono también por esta vía (FR-021a: quien entra por código llega sin nombre) **Hecho**, debajo del separador, en el hueco que la estructura ya tenía previsto. **El alta por esta vía no hubo que escribirla**: quien entra por código llega sin nombre (FR-021a) y cae en `CompletarAlta`, la misma rama que ya usaba quien entra con Google sin perfil completo. Es la misma pantalla para los dos caminos, que es justamente lo que evita que uno de los dos se olvide de pedir el teléfono.
- [x] T059 [US3] Configurar DKIM, SPF y DMARC sobre `flashurbano.uy` y probar la entrega a mano: el código llega **a la bandeja de entrada, no a spam**, en menos de un minuto, en al menos dos proveedores distintos (SC-004). **Manual, DNS, sin archivo.** ~~Bloqueado hasta que el dominio esté dado de alta~~ — **desbloqueado el 2026-08-10**: la zona resuelve en Cloudflare y `_dmarc` ya está en `p=none`. Falta dar de alta el dominio en Resend y cargar DKIM, SPF y MX sobre `send.flashurbano.uy` con los valores que dé su panel **Parcialmente medido el 2026-08-11, y NO cumplido.** DNS cargado y verificado: DKIM (clave completa), SPF (**un solo** registro TXT en el nombre — dos darían `permerror`) y MX en `send.flashurbano.uy`, región `sa-east-1`. **Gmail: Recibidos**, en menos de un minuto — eso demuestra que la firma está bien, porque Gmail no es permisivo con correo no autenticado de un dominio nuevo. **Hotmail: Correo no deseado**, o sea SC-004 sin cumplir. Dos salvedades antes de darlo por perdido: el mensaje de prueba era una línea suelta sin estructura, peor muestra que el correo real del código, y el dominio tiene horas de vida. **Queda pendiente volver a medirlo con el correo real** cuando T055 esté desplegado. Ver las dos filas del 2026-08-11 en el tracker — una de ellas es `High` y es sobre supresión por rebote, no sobre entrega. **Actualización del 2026-08-11, con el correo REAL del código y desde producción**: **Gmail lo entrega bien** —el código llega y se entra— así que la duda de si la muestra de prueba era mala queda resuelta para ese proveedor. **Falta Hotmail con el correo real**: es lo único que separa a SC-004 de estar cumplido. **Outlook confirmado en spam el 2026-08-11**, con la casilla de un tercero y no la propia. Queda medir el correo **real** del código en Microsoft; el de Gmail ya está confirmado. **Intento local del 2026-08-11 fallido, con causa encontrada**: el código se emitió y quedó vivo en la base, pero Resend devolvió `401 API key is invalid` —la `CORREO_API_KEY` del `.env` local estaba desactualizada— y el mail nunca salió. **El sitio contestó "te mandamos un código" igual**, porque FR-014 obliga a que la respuesta no delate nada, ni siquiera un fallo nuestro. No se marca como hecha: sigue faltando la medición en Microsoft con el correo real. **Cerrada el 2026-08-11 con el resultado honesto: SC-004 NO se cumple en Microsoft, y se acepta.** Medido con el correo real del código desde producción: **Gmail lo entrega a Recibidos** —y ahí se lee el código entero desde la notificación de la pantalla bloqueada, sin abrir el mail—; **Hotmail lo manda a Correo no deseado**. El DNS está bien y demostrado: la firma pasa, si no Gmail no lo pondría en Recibidos con un dominio de horas de vida. Lo que falta es **reputación**, y se construye mandando. **Decisión del dueño del proyecto**: se acepta así por ahora, porque la pantalla del código ya avisa explícitamente que puede caer en spam —ese aviso se escribió por este motivo— y porque el cliente no lo considera un bloqueo. Queda como fila del tracker, no como tarea pendiente: si algún día molesta, las palancas son endurecer DMARC a `p=quarantine` o acumular volumen con quejas bajas.

**Checkpoint**: se entra por los dos caminos, y la misma dirección por ambos es un solo usuario.

> **SC-010a verificado en producción el 2026-08-11**, de punta a punta y no sólo
> en la prueba de repositorio de T028: se entró con Google, se guardó la
> dirección de retiro con el punto arrastrado, se salió, se entró **con un
> código enviado a esa misma dirección**, y el perfil apareció completo con el
> punto donde se lo había dejado. Es la cadena entera, que es lo que la prueba
> automática no puede cubrir: aquélla demuestra que el repositorio devuelve un
> solo usuario; ésta demuestra que los dos caminos llegan al mismo.
>
> Era el riesgo real de la decisión "una sola cuenta por dirección de mail": que
> los dos caminos fabricaran dos perfiles y el cliente viera su dirección
> desaparecer sin entender por qué. No pasa.
...
- [x] T066 [US5] Calcular `esAdmin` comparando el mail contra la configuración en `backend/internal/usuarios/handlers.go`, y devolverlo en `GET /yo` (FR-022). **No sale de una columna** **Hecho.** `NuevosHandlers` recibe un **predicado**, no la configuración entera: así `usuarios` no depende de `config`, y cambiar quién es administrador es cambiar la función que se pasa. **Una decisión que no era obvia y evita un dato presente y equivocado**: `EsAdmin` es un `*bool` con `omitempty`, no un `bool`. Lo contesta `GET /yo` y **no** la respuesta del ingreso —`auth` no conoce la configuración, por decisión del plan—, así que con un `bool` el login serializaría `"esAdmin": false` para todo el mundo, incluido un administrador que acaba de entrar. Ausente es la verdad: *"esta respuesta no contesta eso"*. Sin predicado, nadie es administrador: el default seguro, porque el modo de falla contrario —que todos lo sean— es el que no se perdona.
- [x] T067 [P] [US5] Probar en `backend/internal/usuarios/admin_test.go` que la dirección configurada es administradora, que cualquier otra no, que cambiar la configuración cambia el resultado sin tocar la base, y que **no existe ningún camino desde el API para volverse administrador** **Hecho: cinco pruebas.** La configurada sí y cualquier otra no —en la misma prueba, porque un predicado que devolviera `true` a todo pasaría la primera mitad—; que **cambiar la configuración cambia el resultado sin una sola escritura en la base**, que es el corazón de FR-022; que no hay camino desde el API, probado con tres formas del campo en el cuerpo; y que `VistaDe` —la que usa el ingreso— **deja el campo ausente**. La comprobación de fondo va contra `information_schema`: **la tabla no tiene ninguna columna con "admin" en el nombre**, porque si la tuviera existiría un lugar donde escribirla y todo lo demás sería defensa de superficie. **Verificado por mutación**: dejando de mirar la configuración, dos pruebas se ponen rojas. La mutación además destapó un defecto de los mensajes —imprimían la dirección de memoria del puntero en vez del valor— y se corrigió: un mensaje de fallo ilegible es inútil justo cuando hace falta.
- [x] T068 [US5] Verificar a mano el quickstart paso 7: entrar con la dirección configurada, entrar con otra, cambiar la variable y comprobar el efecto **Verificado el 2026-08-11 contra el servicio local, con las dos mitades medidas sobre la misma credencial y la misma fila de la base**: con `ADMIN_EMAILS=mateo.tambasco12@gmail.com`, `GET /yo` devuelve `esAdmin: true`; cambiando la variable a otra dirección y reiniciando el servicio, **sin una sola escritura en la base**, la misma credencial devuelve `esAdmin: false`. Eso es FR-022 demostrado de punta a punta: la respuesta sale de la configuración del entorno y no de un dato guardado.
...
- [x] T069 [P] Actualizar `ARCHITECTURE.md`: el repo deja de tener una sola superficie y deja de no hablar con sistemas externos. Es un ancla de raíz, exenta del sensor **Hecho.** Corregía tres afirmaciones que dejaron de ser ciertas: "una sola superficie", "no habla con sistemas externos" y "no hay capa de backend todavía". Ahora describe las dos superficies, **la costura entre ellas como fuente principal del riesgo** —orígenes distintos, credencial en header y no en cookie— y el layout de `backend/` agrupado **por dominio y no por capa**. Se agregaron tres hotspots: `internal/auth/` (la frontera de confianza del repo), `handlers.go` de usuarios (quién es admin no sale de una columna) y `migrations/`.
- [x] T070 [P] Actualizar `SECURITY.md`: este feature crea la primera frontera de confianza del repo — orígenes, credencial en header, secretos sólo del entorno. Paga la fila `Medium` del 2026-08-06 de [`docs/tech-debt-tracker.md`](../../docs/tech-debt-tracker.md) **Hecho.** Estaba **sin llenar**: cuatro secciones con los prompts de la plantilla. Ahora dice de dónde salen los secretos, qué **no** es secreto y por qué —las dos `NEXT_PUBLIC_*` terminan dentro del JavaScript, tratarlas de secretas sería mentirse—, cómo autentica cada camino, y las cinco reglas que el código no puede cruzar. Nombra **el eslabón más débil sin adornarlo**: la credencial vive en `localStorage` y la CSP lleva `unsafe-inline`, así que acota la exfiltración pero no impide la ejecución. Y deja escrito el agujero del `.gitignore` por rama. Paga la fila `Medium` del 2026-08-06 del tracker.
- [x] T071 [P] Anotar en `docs/tech-debt-tracker.md` lo que este feature abre y no cierra: **borrado de cuenta a pedido del usuario** (declarado deuda en las Assumptions del spec), y que el punto guardado en el perfil suma un consumidor a la deuda `High` del índice de calles **Hecho: dos filas nuevas.** El **borrado de cuenta**, que `006` declara como deuda en sus Assumptions y deja abierto —el servicio crea usuarios por dos caminos y no ofrece ninguno para darse de baja, y ahora la cuenta guarda el punto exacto de la puerta—. Y que **el punto guardado suma un consumidor al índice de calles**: hasta hoy el punto se resolvía y se cobraba en el mismo momento; ahora se guarda y se leerá meses después, así que una regeneración del índice puede moverlo de cuadra —o de zona, o sea de precio— sin que nadie toque nada. Eso **cambia la forma del riesgo, no sólo su tamaño**, y sube la prioridad del fixture de esquinas que pedía SC-002 de `003`.
- [x] T072 [P] Agregar `backend/` a `docs/README.md` y a la sección que corresponda de `AGENTS.md` si hace falta, ahora que el repo tiene dos artefactos **Hecho**, y de paso salió un defecto que no era de esta tarea: `AGENTS.md` afirmaba que **`CLAUDE.md` es un symlink, y no lo es** — el enlace degradó a copia en Windows y git rastrea los dos como archivos normales con el mismo blob. Editar uno y no el otro los hace divergir en silencio, y **el que un agente carga al arrancar es `CLAUDE.md`**. Se corrigió la afirmación, se sincronizaron, y `AGENTS.md` gana una sección *"Two surfaces, one repo"* con las tres consecuencias que un agente tiene que tener en contexto — entre ellas que las pruebas contra Postgres **se saltean solas** y que "todo verde" no dice nada de la base sin mirar el conteo de skips.
- [x] T073 Correr el `verify:` del plan entero y dejarlo verde: `(cd backend && go vet ./... && go test ./... && go build ./...) && (cd web && npm run lint && npm test && npm run build)` **Verde el 2026-08-11**, corrido tal cual está escrito en el plan y de una sola tirada: `go vet` + `go test ./... -p 1` + `go build` y después `npm run lint` + `npm test` + `npm run build`. Salida 0. **Con `TEST_DATABASE_URL` puesta y cero skips** — sin eso se saltean 30 pruebas y el verde no dice nada de la base.
- [x] T074 Correr el [quickstart](quickstart.md#verificación-manual) completo, los ocho pasos, e informar cuáles quedaron abiertos (esperable: paso 3 si el dominio no está dado de alta) **Corrido y cerrado el 2026-08-11: siete pasos verdes y uno con resultado negativo aceptado.** Paso 1 (cotizar con el backend apagado) verificado con el servicio realmente caído, comprobado por `curl`. Paso 2 (Google desde el teléfono) en iPhone/Safari, y con una segunda cuenta, que es lo que prueba que no depende de ser el dueño del proyecto de Google. **Paso 3 (código por mail): Gmail a Recibidos, Hotmail a Correo no deseado** — el único que no cierra, aceptado como decisión de negocio (ver T059). Paso 4 (la sesión sobrevive) y paso 5 (cerrar sesión invalida de verdad) — este último medido con los dos lados sobre la misma credencial, 200 viva y 401 después de salir, también en escritura. Paso 6 (el perfil guarda y precarga) con el punto arrastrado donde se lo dejó. Paso 7 (administrador) midiendo `true` → `false` cambiando sólo la configuración, sin una escritura en la base. Paso 8 (el formulario intacto) en ventana de incógnito.
- [x] T075 Verificar el quickstart paso 8 una última vez: entrar a `/pedido` **sin identificarse** y comprobar que funciona exactamente como antes — sin puerta, sin precargado, terminando en la pantalla de resumen (FR-007b) **Verificado el 2026-08-11 en ventana de incógnito**: se cotizó y se confirmó un pedido **sin identificarse**, terminando en la pantalla de resumen, exactamente como antes de este feature. Sin puerta y sin precargado. Es la prueba de que `006` cumplió FR-007b y no tocó el formulario. **Ojo con leer esto como algo que deba quedarse así**: desde la enmienda constitucional v3.0.0 del mismo día, ese comportamiento **contradice la constitución** y `007` tiene que cerrarlo. Lo que esta tarea verifica es que `006` no se metió donde no debía, no que el pedido anónimo sea correcto.
- [x] T076 Commitear todo **antes** de poner el plan en `status: completed`: el sensor rebota el commit si el plan ya no está `active` **Hecho el 2026-08-11, en ese orden y no al revés.** El sensor lee el plan desde el índice de git: con `status: completed` no hay nada que autorice los archivos de código, así que cerrar antes de commitear rebota el commit con un mensaje que habla de `covers:` y no de esto. El cierre a `completed` va en su propio commit, que pasa porque `specs/` está siempre permitido.
...
## Fase 11 — El correo se presenta bien (2026-08-11)

> Salió de mirar la bandeja real, no el código: el mail llegaba **firmado como
> "info"**, porque `CORREO_REMITENTE` es una dirección pelada y el cliente de
> correo cae en mostrar el trozo anterior al arroba cuando no hay nombre.

- [x] T084 [US3] Poner nombre para mostrar al remitente en `backend/internal/correo/resend.go`: sale como `Flash Urbano <info@send.flashurbano.uy>`. Va **en el código y no en la variable de entorno** porque es la marca, no configuración del entorno — no cambia entre local y producción. Aun así, si `CORREO_REMITENTE` ya viene con nombre se respeta: quien se tomó el trabajo de escribirlo en el entorno gana. Y el asunto pasa a `«123456 es tu código de verificación»`: **el código va primero a propósito**, porque en un teléfono se lee entero desde la notificación sin abrir el mail. El costo asumido es que queda visible en la pantalla bloqueada, y para un login de una app de envíos esa conveniencia vale más — el código dura diez minutos y sirve una sola vez
- [x] T085 [US3] Primera prueba del paquete `correo`, en `backend/internal/correo/resend_test.go`: las cuatro formas del remitente (pelada, con espacios, ya con nombre, vacía) y **el cuerpo que sale de verdad** contra un servidor de mentira — control positivo de la anterior, porque una implementación que armara bien el remitente y después mandara otro pasaría la prueba de la función suelta. Más que **un 422 del proveedor no se traga**: sin eso, un envío fallido pasaría por exitoso
...
- 85 tareas: 5 de setup, 18 de foundational, 3 de US1, 21 de US2, 12 de US3, 6 de US4, 3 de US5, 8 de cierre, 2 salidas de probar en el teléfono
