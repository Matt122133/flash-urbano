...
- [x] T024 Cerrar la fila del tracker del 2026-08-26 sobre el diseño de la app — es la que este feature vino a pagar. **Cerrarla solo si T028 confirma**, no antes **HECHO**: la fila pasa a `Resolved`, **con lo que queda abierto escrito adentro** en vez de darla por saldada entera. Lo que la fila pedía —alcance del pulgar, tamaño de toque, paleta de marca, devolución visible— está entregado y probado con una mano. Lo que no: el contraste al sol, que sigue sin evaluar.
...
- [x] T027 Ejecutar el **nivel 2 completo** en el emulador: Q3 a Q8. **Es obligatorio y es donde se encuentran los defectos de este feature**; `012` demostró que compilar no dice nada. Sacar capturas para comparar con las de T002 **PARCIAL al 2026-08-30**: Q3 (las tres pestañas), Q4 (mover y deshacer), Q6 (nada de morado, nada tapado), Q7 (vacíos y sin señal) y Q8 (que paso a ser prueba de JVM) **hechos**, con capturas. **Q5 CERRADA el 2026-08-30 por Mateo**, en un teléfono real (Xiaomi con Android 16, la misma familia que el de Diego): lo probó con una mano y **llegó a todo**. Es el criterio central del feature y el que `012` dejó sin evaluar
- [x] T028 **TUYA** Ejecutar el **nivel 3** ([quickstart](quickstart.md) Q9 y Q10): generar el APK firmado, pasárselo a Diego —`adb install` **no funciona** en su Xiaomi—, y que haga una jornada real. **Es SC-008 y es lo único que puede cerrar este feature.** `012` dejó estos dos pasos sin evaluar y de ahí salió este trabajo **CERRADA PARCIAL el 2026-08-30.** Q9 hecho: APK de `release` generado, **firmado** y **sin la excepción de texto plano**, comprobado sobre el manifiesto del APK compilado. Instalado en un teléfono real y abierto. **Q10 lo hizo Mateo, no Diego**: probó el alcance del pulgar con una mano y llegó a todo. **La lectura al sol sigue sin evaluar**, y Diego todavía no la vio — las dos cosas quedan para cuando le llegue el APK, y no se tildan acá. **Hallazgo del proceso**: `adb install` **SÍ funcionó** en este Xiaomi. O sea que la traba del teléfono de Diego no es de HyperOS en general, es la opción *Instalar vía USB* que él tiene apagada — corregido en `docs/processes/app-repartidor.md`.
- [x] T029 Poner `specs/015-diseno-de-la-app/plan.md` en `status: completed` **después** de commitear el resto **HECHO el 2026-08-30**, después del commit `d649a68`.
