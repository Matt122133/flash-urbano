...
- [x] T022 **TUYA** Desplegar la migración a producción y comprobar que **el servicio arranca**. Es el paso que el 2026-08-12 se dio por sentado **HECHO, comprobado el 2026-08-31 y no supuesto.** El despliegue vivo en Railway es el commit `a4dbd9b` —el merge del PR #29, o sea `016` entero— con `status: SUCCESS`. Como las migraciones **se aplican al arrancar** (`backend/README.md`, research D7), un servicio Online es prueba de que `0006` corrio: si hubiera fallado, no habria arrancado. Y `GET /salud` contra el dominio publico devuelve `{"estado":"ok","base":"ok"}`, que ademas confirma que la base responde. Ese mismo despliegue es el que lleva el arreglo del deshacer que T023 dejaba pendiente de redespliegue.
- [x] T023 **TUYA** Nivel 3, Q9: que Diego lo use en una entrega real, incluyendo una a un tercero. Dos preguntas que solo contesta él: si los dos toques le alcanzan con las manos ocupadas, y **si pedir la cédula le resulta natural o incómodo frente a la persona**. Lo segundo puede cambiar el diseño **PARCIAL al 2026-08-31.** Mateo lo probo en un telefono real **contra produccion**. Lo que anda: los dos caminos, y **la web muestra el nombre de quien recibio y NO la cedula** — SC-004 comprobado sobre el producto real y no sobre una prueba. **DEFECTO QUE REPORTO**: al tocar el campo del nombre, el teclado tapaba la hoja y habia que **subirla con el dedo**. Causa: la hoja abria a media altura, asi que `imePadding` corria el contenido dentro de un contenedor que seguia midiendo media pantalla. Corregido con `skipPartiallyExpanded`. **SEGUNDO DEFECTO QUE REPORTO**: al deshacer una entrega, el pedido volvia a Pendientes **y seguia mostrando quien lo habia recibido**. La consulta buscaba el ultimo cambio a `entrega` sin mirar si el pedido seguia entregado. Corregido, con prueba de regresion que **se comprobo en rojo** sin el arreglo. **La fila del historial NO se borra**: que esa persona recibio el paquete es un hecho que ocurrio, y esa tabla existe para no perderlo — lo que se corrigio es mostrarlo cuando ya no corresponde. **El arreglo del teclado quedo CONFIRMADO por Mateo el 2026-08-31**, en el mismo telefono donde se vio el defecto. **Falta**: que el arreglo del deshacer se vea en produccion —es del backend, asi que necesita el merge y el redespliegue— y la pregunta que solo contesta Diego: si pedir la cedula se siente natural o incomodo frente a la persona.
- [x] T024 Poner `specs/016-quien-recibio/plan.md` en `status: completed` **después** de commitear el resto **HECHO el 2026-08-31.**
