...
- [x] T023 **TUYA** Nivel 3, Q9: que Diego lo use en una entrega real, incluyendo una a un tercero. Dos preguntas que solo contesta él: si los dos toques le alcanzan con las manos ocupadas, y **si pedir la cédula le resulta natural o incómodo frente a la persona**. Lo segundo puede cambiar el diseño **PARCIAL al 2026-08-31.** Mateo lo probo en un telefono real **contra produccion**. Lo que anda: los dos caminos, y **la web muestra el nombre de quien recibio y NO la cedula** — SC-004 comprobado sobre el producto real y no sobre una prueba. **DEFECTO QUE REPORTO**: al tocar el campo del nombre, el teclado tapaba la hoja y habia que **subirla con el dedo**. Causa: la hoja abria a media altura, asi que `imePadding` corria el contenido dentro de un contenedor que seguia midiendo media pantalla. Corregido con `skipPartiallyExpanded`. **SEGUNDO DEFECTO QUE REPORTO**: al deshacer una entrega, el pedido volvia a Pendientes **y seguia mostrando quien lo habia recibido**. La consulta buscaba el ultimo cambio a `entrega` sin mirar si el pedido seguia entregado. Corregido, con prueba de regresion que **se comprobo en rojo** sin el arreglo. **La fila del historial NO se borra**: que esa persona recibio el paquete es un hecho que ocurrio, y esa tabla existe para no perderlo — lo que se corrigio es mostrarlo cuando ya no corresponde. **Falta que Mateo confirme los dos arreglos en el telefono, y la pregunta que solo contesta Diego**: si pedir la cedula se siente natural o incomodo frente a la persona.
