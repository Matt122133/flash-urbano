...
- [x] T003 Escribir la migración `backend/migrations/0003_pedidos.sql` con la secuencia `pedidos_codigo_seq` y la tabla `pedidos` completa de [data-model.md](data-model.md), incluidos los tres índices. **`ON DELETE RESTRICT`** en `usuario_id`, no `CASCADE`; **`retiro_punto` NOT NULL**; `estado` y `paquete_tamano` como `text` con `CHECK`, no enums; `retiro_fecha date` + `retiro_hora time`, **no** un `timestamptz` (research D8). Sólo hacia adelante: no hay migración de vuelta
- [x] T004 Implementar el repositorio en `backend/internal/pedidos/pedido.go`: `Crear` (con el `INSERT … ON CONFLICT (usuario_id, clave_idempotencia) DO NOTHING` y la relectura que devuelve el existente), `PorUsuario` y `Todos`. El código de pedido lo genera la base por `DEFAULT`, no el servicio
...
- [x] T017 [US2] Agregar a `web/lib/api.ts` las funciones `crearPedido` (con la cabecera `Idempotency-Key`) y `misPedidos`. Sin dependencias nuevas: `ErrorApi` ya distingue `sinRespuesta` de `sesionInvalida`, que es todo lo que la pantalla necesita
- [x] T018 [P] [US2] Probar las funciones nuevas en `web/lib/api.test.ts`, incluido que la cabecera de idempotencia viaje y que un `200` y un `201` se traten igual desde el punto de vista de quien llama
- [x] T019 [US2] Modificar `web/components/pedido-form.tsx` para que reciba `onConfirmar` como prop y exporte el tipo de sus datos. **No agrega ni un import hacia `lib/api.ts` ni `lib/sesion.ts`**: el formulario deja de saber que existe un servicio
- [x] T020 [US2] Escribir `web/components/pedido/crear-pedido.tsx`: el componente cliente que importa `lib/api.ts`, genera la clave de idempotencia con `crypto.randomUUID()` en un `useRef` y monta `<PedidoForm onConfirmar={…} />`. La clave se comparte entre reintentos y se descarta después de un pedido creado
- [x] T021 [US2] Modificar `web/app/pedido/page.tsx` para montar `crear-pedido.tsx` en vez del formulario directo. La página sigue siendo de servidor: sólo metadata y encabezado
- [x] T022 [US2] Agregar a `web/lib/cotizar-abierto.test.ts` el **control positivo**: afirmar que `components/pedido/crear-pedido.tsx` **sí** alcanza `lib/api.ts`. Sin él, borrar el envío entero deja la guarda verde y nadie se entera. **No se tocan `ENTRADAS` ni `PROHIBIDOS`**
- [x] T023 [US2] [MANUAL] **Romper la guarda a propósito**: agregar `import { pedir } from "@/lib/api";` en `pedido-form.tsx`, correr `npx vitest run lib/cotizar-abierto.test.ts`, **ver el rojo**, y deshacerlo. Una guarda negativa que nadie vio fallar no está demostrada
- [X] T024 [US2] [MANUAL] Verificar M1 de [quickstart](quickstart.md#m1--cotizar-sigue-abierto-con-el-servicio-apagado-sc-001): con el backend **detenido**, cotizar de punta a punta sin un solo error y con **cero llamadas fallidas** en la pestaña de red. Es US1, y se corre acá porque acá es donde se pudo romper
...
- [X] T031 [US3] [MANUAL] Verificar M2 en un **teléfono**: formulario entero, un solo toque en confirmar, ingreso con Google, código `FU-####`. Comprobar que la URL **no cambió**, que no se reescribió ni un campo, y que el pin quedó donde se lo había arrastrado — *hecho el 2026-08-14 en Safari de iPhone, contra el export estático servido por LAN. `FU-0002` creado; URL sin cambios, ningún campo reescrito, el pin donde se lo dejó. **El ingreso fue por código de mail, no por Google**, y no por comodidad: la consola de Google no acepta una IP como origen autorizado, así que desde `http://192.168.1.4:3000` el ingreso con Google no puede funcionar. No debilita la verificación — lo que M2 mide es la puerta y la reanudación, no cuál de los dos ingresos se usó, y Google en teléfono ya quedó verificado en `006` (T046/T047)*
- [~] T032 [US3] [MANUAL] **DIFERIDA el 2026-08-14, por decision del dueño del proyecto.** La consola de Safari en iPhone necesita un Mac y un cable, que no hay. Lo que queda sin comprobar es concreto y conviene no perderlo: que el borrador —con el nombre y el telefono de un tercero— no quede escrito en el disco del telefono. El diseño lo sostiene (el ingreso ocurre en un dialogo, justamente para no persistir nada) pero **nadie lo midio**. Verificar M3: `Object.keys(localStorage)` y `Object.keys(sessionStorage)` **no contienen el borrador**, en particular el nombre y el teléfono de quien recibe. Es la razón por la que se eligió el diálogo — un tercero que no consintió nada no puede quedar escrito en el disco de un teléfono compartido
- [X] T033 [US3] [MANUAL] *(hecho en dos partes: el 2026-08-14 se comprobo en pantalla que desistir deja lo cargado intacto, y el 2026-08-15 se cerro la mitad que importa —`GET /admin/pedidos` no muestra ningun pedido nuevo—, consultada contra el servicio y no mirando la pantalla)* Verificar M4: desistir del diálogo deja lo cargado y **`GET /admin/pedidos` no muestra ningún pedido nuevo**. Que la pantalla no diga nada no prueba que no se creó
- [X] T034 [US3] [MANUAL] Repetir M2 entero en **Safari de iPhone**. `006` encontró que el modo de falla predecible es que un navegador se comporte distinto — *hecho el 2026-08-14, **y es la misma corrida que T031**, no una segunda. Se cumple al pie de la letra —M2 entero en Safari de iPhone— pero conviene decir qué queda sin cubrir: la intención de tener dos tareas era ejercitar **dos navegadores de teléfono**, y sobre teléfono sólo se probó Safari. Chrome de Android no se corrió. El riesgo que `006` nombra —que un navegador se comporte distinto— queda cubierto sólo entre escritorio (Chrome) y teléfono (Safari)*
...
- [x] T035 [US4] Implementar en `crear-pedido.tsx` la precarga desde `GET /yo`: nombre, teléfono y dirección de retiro con apto, cooperativa y punto. Un perfil **sin** dirección deja el formulario utilizable y vacío en esa parte, sin errores (FR-025)
- [x] T036 [US4] Implementar la revalidación del punto guardado (FR-022, research D7): reconstruir la esquina, comprobar que el punto caiga en la cuadra declarada, y **si no cae**, descartarlo, recolocar en el cruce resuelto y avisar. Nunca se cobra en silencio sobre un punto que ya no corresponde. Es la contraparte que `FR-019b` de `006` dejó pendiente
- [x] T037 [US4] Reutilizar en esa reconstrucción el manejo de **calles homónimas** que ya resuelve `formulario-perfil.tsx` —probar todas las combinaciones y desempatar con el punto guardado—. El índice tiene 50 grupos de nombres que se normalizan igual, y `callePorNombre` devuelve el primero, que puede ser el equivocado. **No escribir esa lógica de nuevo**
- [x] T038 [US4] Implementar que la precarga **no pise lo que la persona ya escribió** (FR-007). Con la puerta a mitad de formulario, identificarse es un momento en que la precarga podría reescribir una dirección recién tipeada — un caso que antes de este feature no existía
- [X] T039 [US4] [MANUAL] **CERRADA el 2026-08-15, las dos mitades verdes sobre el build con el arreglo.** Primera mitad verde el 2026-08-14 (con sesion, la precarga trae todo y `/perfil` no cambia al pedir). **La segunda mitad encontro un defecto real y esta pendiente de re-verificar**: escribir una direccion distinta de la guardada y despues identificarse desde el dialogo **borraba todo lo tipeado y lo reemplazaba por el perfil** — FR-007 violado. La causa no era que la precarga corriera dos veces sino que `PedidoForm` se **desmontaba y volvia a montar** al cambiar la sesion. Arreglado el mismo dia en `crear-pedido.tsx`; **la comprobacion hay que rehacerla entera sobre el build nuevo**. Verificar M5 completo, incluidas **las dos direcciones**: que el perfil no cambie al pedir, y que escribir una dirección distinta de la guardada y después identificarse deje el pedido con la que se escribió
- [X] T040 [US4] [MANUAL] *(resuelto el 2026-08-15: **alcanzo sin modificarlo**. `bloque-direccion.tsx` no se toco en todo el feature y la precarga con direccion guardada funciona, asi que `covers:` NO hubo que extenderlo por este motivo. La deuda conocida del campo `Esquina` deshabilitado sigue abierta en el tracker: no molesto en este flujo porque la precarga trae la direccion completa y no hay que reelegir la calle)* Comprobar si `bloque-direccion.tsx` alcanza sin modificarlo. Si no —la deuda conocida del campo *Esquina* deshabilitado con una dirección precargada—, **extender `covers:` en el plan con el motivo escrito** en vez de arreglarlo de paso
...
- [X] T046 [MANUAL] *(hecho el 2026-08-14: `/pedido` y la confirmacion leidas texto por texto, sin promesas de contacto y sin el enlace a WhatsApp)* Verificar M6: leer `/pedido` y la confirmación **texto por texto** contra SC-012. Es la verificación que se olvida
- [X] T047 [MANUAL] *(hecho el 2026-08-14. Con el servicio caido, confirmar fallo diciendo que el pedido NO se creo y lo cargado quedo en pantalla; levantado el servicio, el reintento creo `FU-0006`. **La ausencia de duplicado se midio en la base**, no se supuso: las seis filas tienen seis claves de idempotencia distintas, y `FU-0005` es otro pedido —otra entrega, otro destinatario—, no una copia)* Verificar M7: con el servicio caído, confirmar falla diciendo que el pedido **no** se creó, lo cargado sigue en pantalla, y al levantarlo confirmar de nuevo deja **un** pedido (SC-013)
- [X] T048 [MANUAL] *(hecho el 2026-08-14: un punto fuera de las cinco zonas no deja confirmar y ofrece el boton que lleva a `/contacto`)* Verificar M8: un punto de retiro fuera de las cinco zonas **no permite confirmar desde el sitio** y deriva a contacto directo (SC-009, FR-020). **No** hay que verificar que el servicio lo rechace: no lo hace, y FR-020a lo dice explícitamente
...
- [x] T055 Con un pedido confirmado en pantalla, retirar el encabezado de `/pedido` — título e instrucción — y devolverlo al volver al formulario (FR-034). El encabezado se le **pasa** a `CrearPedido` desde `page.tsx`: el copy se queda en su archivo y sigue entrando en el HTML pre-renderizado, que es lo que lee un buscador
