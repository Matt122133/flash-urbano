...
- [x] T003 **Averiguar qué versión de Android tiene el teléfono de Diego** (research D3). El APK va a exigir 8.0 o superior. **Es una pregunta para una persona, no una tarea de código**, y si la respuesta fuera menor a 8 hay que volver al plan antes de escribir nada. La respuesta se anota acá mismo cuando llegue. **MEDIDA el 2026-08-26 con el telefono conectado**: Redmi, **Android 16 (API 36)**, muy por encima del piso de 26. Estuvo como suposicion casi toda la jornada y resulto correcta, pero el dato es este
...
- [x] T018 [US3] Comprobar que esto **no rompe la web**, que usa la misma sesión: `go test ./...` verde y el sitio sigue entrando normal. **HECHO el 2026-08-26**: `go test ./...` verde, 164 pruebas y 0 skips. Y el camino propio de la web comprobado contra el backend local con una sesion vieja: **`GET /yo` y `GET /pedidos` contestan 200** —son las dos llamadas que el sitio hace al cargar— y esa sesion **tambien movio su vencimiento sola**, de un dia a veintiocho. Lo unico que no se hizo es abrirlo en un navegador, que a esta altura solo agregaria el render, ya cubierto por las pruebas de `web/`
...
- [x] T019 [P] [US1] Modelar en Kotlin la respuesta de `GET /admin/pedidos` con `kotlinx.serialization`. **Los tres puntos son opcionales** (data-model §5): un pedido puede venir sin punto de retiro, sin punto de entrega, o con un estado desconocido. Modelarlos como obligatorios rompe la app con datos que hoy existen en producción
- [x] T020 [P] [US1] Probar ese mapeo en JVM con las tres formas: pedido completo, sin punto de retiro, y sin punto de entrega. **Es lo único que se puede probar sin dispositivo, y es justo donde estuvo el defecto en `011` las dos veces**
- [x] T021 [US1] Cliente del servicio con OkHttp: traer la lista con la credencial, y traducir un fallo de red en algo que la pantalla pueda mostrar — **no en una excepción que la tumbe**
- [x] T022 [US1] Pantalla de ingreso: mail, código, y guardar la credencial en `DataStore` (research D6). Se ve **una vez en la vida**
- [x] T023 [US1] Pantalla principal con **dos secciones, Pendientes y Tomados** (FR-013), y cada pedido mostrando código, las dos direcciones, tamaño y cantidad, y **los dos teléfonos** —quien envía y quien recibe (FR-015)—, tocables para llamar
- [x] T024 [US1] **La sección de Entregados, fuera de la pantalla principal** (FR-013, contrato §4.3). Es la única lista que crece sin límite y sirve para consultar, no para trabajar. **Sin esta tarea los pedidos entregados desaparecen sin dónde verse** — lo encontró el analyze del 2026-08-23
- [x] T025 [US1] Resolver los estados que no son una lista (contrato §5): sin señal al abrir dice que no pudo y ofrece reintentar —**nunca una lista vacía**, que parecería que no hay trabajo—, y sin pedidos de verdad dice otra cosa distinta
- [x] T026 [US1] Ejecutar **E1 y E2** de [quickstart.md](quickstart.md) en el emulador. **HECHO el 2026-08-26** contra el backend local, con ingreso real por codigo. E1: se ven los tres pedidos con codigo, las dos direcciones, tamano, cantidad y los dos telefonos, agrupados en Pendientes y Tomados. E2: la homonima (Valencia x Zaragoza), el sin punto de retiro y el sin punto de entrega se dibujan los tres con su direccion escrita. **Encontro dos defectos que ninguna prueba de JVM iba a ver**: el texto del boton de ingreso cortado —`padding` puesto despues del `height`— y el titulo pegado a la barra de estado. Los dos corregidos. **Y un hallazgo de entorno**: la base de dev tenia `entrega_punto NOT NULL`, o sea el `0004` de antes de la correccion del 2026-08-23, porque una migracion ya registrada no vuelve a correr. Se alineo a mano. E2 es ⚠ y exige fabricar el pedido sin punto de entrega a mano, porque ya no se puede crear uno así
...
- [x] T033 `verify:` verde en las **tres** superficies, con `TEST_DATABASE_URL` puesto. **Mirar el conteo de skips de Go**: sin esa variable el paquete que este feature cambia se saltea entero y el verde no dice nada **HECHO el 2026-08-26**: verde en las tres superficies con `TEST_DATABASE_URL` puesto — web 147, Go **164 con 0 skips**, Android 30.
- [x] T034 Ejecutar **E8 y E9**: E8 comprueba con `curl` que **ni leer ni escribir pedidos funciona sin credencial** (SC-006), y E9 que la sesión mueve su vencimiento **HECHO el 2026-08-26.** E8: `GET` y `PATCH` dan **401 sin credencial y 401 con una inventada**, el pedido no se movio, y la respuesta no filtra ningun dato. E9: se envejecio la sesion a un dia de vida restante, se uso la app, y `expira_en` volvio a ~28 dias sola.
- [x] **T035 — TUYA** **Agregar el mail de Diego a `ADMIN_EMAILS` en producción** (variable del servicio en Railway). Hoy esa lista tiene la dirección del dueño del repo: **sin este paso Diego ingresa y no ve ni un pedido**, y es la clase de detalle que aparece con él esperando. Lo encontró el analyze del 2026-08-23 **HECHO**: lo confirmó Mateo el 2026-08-30 — el mail de Diego quedó en la variable, y la app ya venía viendo pedidos reales contra producción (T038/T4), que es la comprobación de que la lista quedó bien
- [x] T036 Generar el APK de `release` y **comprobar que NO lleva la excepción de texto plano** (research D5) mirando el manifiesto del APK compilado. Confiar en que la configuración quedó bien es justo cómo se cuela. Comprobar antes que la respuesta de T003 sea 8.0 o superior **HECHO el 2026-08-26**, y con control positivo: el manifiesto del APK de `release` **no** lleva `networkSecurityConfig` ni `usesCleartextTraffic`, y el de `debug` **si** — o sea que la comprobacion sabria detectarlo. `BASE_URL` de release apunta a Railway. **Encontro ademas que el APK salia SIN FIRMAR**: research D11 dice que se firma con la clave de depuracion y el `build.gradle.kts` no tenia `signingConfig`, asi que `assembleRelease` producia un archivo que Android no instala. Corregido y comprobado instalandolo.
- [x] T037 Instalar la app **en el teléfono de Diego**. Con el teléfono conectado por USB y depuración activada se instala con `adb`; si no, se le pasa el archivo y hay que habilitar "instalar aplicaciones desconocidas" una vez **HECHO el 2026-08-26.** **`adb install` NO funciono**: HyperOS lo rechaza con `INSTALL_FAILED_USER_RESTRICTED`, que exige la opcion "Instalar via USB" y con ella cuenta Mi. Se instalo empujando el APK a Descargas y abriendolo desde el gestor de archivos. Todo el procedimiento y las vias descartadas quedaron en [`docs/processes/app-repartidor.md`](../../docs/processes/app-repartidor.md)
- [x] **T038 — CERRADA PARCIAL el 2026-08-30, con lo que falta mudado al tracker.** T4 quedó hecho; **T2 y T3 (usarla con una mano, leerla al sol) siguen sin evaluar** y no se tildan como hechos. No se quedan colgando de este plan porque ya viven, con su disparador, en la fila de `docs/tech-debt-tracker.md` del 2026-08-26 sobre el diseño de la app — que es el feature propio donde se van a mirar en serio, junto con alcance del pulgar, tamaño de toque y contraste. Cerrar `012` con esta tarea abierta bloquearía trabajo del cliente por una prueba de usabilidad que pertenece a otro feature. **Estado real: T4 sí, T2 y T3 no.** Texto original abajo — **PARCIAL al 2026-08-26**: **T4 hecho** —la app entro contra produccion y movio estados de verdad, con lo que se cierra tambien "ver un pedido real contra produccion", pendiente desde el 2026-08-11—. **T2 y T3 sin evaluar**: usarla con una mano y leerla al sol. Y de mirarla salio trabajo nuevo, no un tilde: el diseño necesita otra vuelta (ver abajo). Ejecutar **T2, T3 y T4** de [quickstart.md](quickstart.md) con Diego: que se use con una mano, que se lea al sol, y **un pedido real de punta a punta**. T4 cierra además algo pendiente desde el 2026-08-11 — ver un pedido real contra producción
- [x] T039 [P] Escribir en `docs/processes/` **cómo se genera el APK** (FR-011) y **cómo se le corta la sesión a un teléfono perdido** (FR-016), que es borrar su fila de `sesiones` desde la consola de Railway **HECHO**: [`docs/processes/app-repartidor.md`](../../docs/processes/app-repartidor.md). **Una desviacion deliberada**: para el telefono perdido el procedimiento es `UPDATE ... revocada_en = now()` y no `DELETE`. Las dos cortan el acceso de inmediato —`Resolver` filtra por `revocada_en IS NULL` en la misma consulta— pero el `UPDATE` deja el rastro de que paso y cuando; el `DELETE` borra la evidencia del incidente junto con el acceso.
- [x] T040 [P] Actualizar `ARCHITECTURE.md` con la tercera superficie: qué vive en `android/`, que habla con el servicio por HTTP igual que el sitio, y que no comparte código con nada **HECHO**, y ademas se corrigio `AGENTS.md` y su copia `CLAUDE.md`, que seguian diciendo **"dos superficies"** — es lo primero que lee cualquier agente al abrir el repo.
- [x] T041 [P] Anotar en `docs/tech-debt-tracker.md` lo que este feature deja afuera **con su disparador**: la ruta y el panel (esperando que el cliente defina "ruta económica"); la revocación remota de sesión (**se vuelve obligatoria el día que haya un segundo repartidor**); que elegir qué pedidos lleva cada día no queda registrado; y que **`GET /admin/pedidos` devuelve todos los pedidos de todo el mundo sin paginar** —la app los baja enteros cada vez—, con su umbral en números como se hizo en `010` **HECHO**: cinco filas. La de la paginacion lleva el numero **medido**, no estimado: ~710 bytes por pedido, con su disparador en 1.000 filas.
- [x] **T042 — recien cuando cierren T035, T037 y T038.** Poner `specs/012-app-repartidor/plan.md` en `status: completed` **después** de commitear el resto **HECHO el 2026-08-30.** T035 confirmado por Mateo, T037 cerrado el 2026-08-26, T038 cerrada parcial con T2 y T3 mudadas al tracker. El plan se cierra para dejar entrar a `013`, que trae un cambio de producto pedido por el cliente: **no pueden convivir dos planes activos**
