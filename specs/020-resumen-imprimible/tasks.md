...
- [ ] T001 Correr `cd web && npm test` antes de tocar nada y anotar el resultado,
      para que un rojo posterior sea atribuible a este feature.
- [ ] T002 Instalar `jspdf` como dependencia y `fast-png` como dependencia **de
      ...
      declarada.
- [ ] T003 Leer la guia de Next que corresponda en `node_modules/next/dist/docs/`
...
- [ ] T004 Escribir `web/design-source/build-silueta.js`: lee
...
- [ ] T005 Correr el generador y **mirar el PNG**: camion macizo con lineas de
...
- [ ] T006 [P] Documentar el generador en `web/design-source/README.md`, con el
...
- [ ] T007 [US1] Crear `web/lib/etiqueta.ts` con el tipo neutro `Etiqueta`
...
- [ ] T008 [US1] Escribir `web/lib/etiqueta.test.ts` con las guardas que son el
...
- [ ] T009 [US1] **Control negativo de FR-007, obligatorio**: agregar
      ...
      **esa prueba se pone en rojo**, y sacarlo. Una guarda que afirma que algo no
      ...
      del quickstart.
- [ ] T009b [US1] Agregar en `web/lib/etiqueta.test.ts` la guarda del grafo de
...
- [ ] T010 [US1] Crear `web/lib/etiqueta-pdf.ts`: dibuja la hoja A4 con jsPDF a
...
- [ ] T011 [US1] Agregar el boton *Imprimir resumen* al componente `Confirmation`
...
- [ ] T012 [US1] Manejar la falla en ese boton (FR-015): si la libreria no carga
...
- [ ] T019 Comprobar SC-006 con la pestaña de red: navegar el sitio **sin tocar
...
- [ ] T021 [P] Actualizar `ARCHITECTURE.md`: `lib/etiqueta.ts` como el modulo que
...
- [ ] T022 Correr `verify:`: `cd web && npm run lint && npm test && npm run build`.
...
- [ ] T024 Commitear con el plan todavia en `active`, o el sensor de cobertura
